...
//
// Helpers: Craig lecture, TA: Mohammand
//
// Also, list any resources beyond the course textbook and the course pages on Piazza
// that you used in making your submission.
//
// Resources:  YouTube, Stackflow(website)
//
//%% Instructions:
...
public:
	...
	//declarations of shared pointer to constant for all type: int char float
	...
	virtual shared_ptr_to_constant complement() { throw std::domain_error("complement Requires numeric_int_const"); }
	virtual shared_ptr_to_constant mod(constant& rval) { throw std::domain_error("mod Requires numeric_int_const"); }
	virtual shared_ptr_to_constant leftShift(constant& rval) { throw std::domain_error("left Requires numeric_int_const"); }
    virtual shared_ptr_to_constant rightShift(constant& rval) { throw std::domain_error("right Requires numeric_int_const"); }
    virtual shared_ptr_to_constant addition(constant& rval) { throw std::domain_error("+ Requires numeric_int_const"); }
    virtual shared_ptr_to_constant subtraction(constant& rval) { throw std::domain_error("- Requires numeric_int_const"); }
    virtual shared_ptr_to_constant division(constant& rval) { throw std::domain_error("/ Requires numeric_int_const"); }
     // this class has to have all the exception handling for all the operations
	virtual int getIntValue() { throw std::domain_error("int Requires numeric_int_const"); }
...
public:
	...
	shared_ptr_to_constant ofTypeCreateConstant(shared_ptr_to_constant constToCreateFromSP)
	{
		...
		//************there are 3 cases for this. int , char and float**************************88
...
public:
	...
	Token* process_pointer_id_exp() { return process_pointer_id(); }
};
...
public:
	...
	*/
	...
	{ // '='
		...
		//when implement /=   += == should have this switch loop for different cases
        switch (this->stringValue[0]) {
                /* ***** this needs work for other supported data types i.e. char and float */
            case '/': // '/='
                return evalMap[key].sptc = evalMap[key].sptc->division(*evalMap[key].ts.ofTypeCreateConstant(right));
            case '+': // '+='
                return evalMap[key].sptc = evalMap[key].sptc->addition(*evalMap[key].ts.ofTypeCreateConstant(right));
            case '-': // '-='
                return evalMap[key].sptc = evalMap[key].sptc->subtraction(*evalMap[key].ts.ofTypeCreateConstant(right));
       //***************************************************************************************88
        }
...
public:
	...
	{
		/* ***** this needs work */
		if (evalChildToken(0)->getIntValue() == 1)
...
public:
	...
	shared_ptr_to_constant eval()                   //shift operation
	{
		/* ***** this needs work */
		if (this->stringValue[0] == '<')
			return evalChildToken(0)->leftShift(*evalChildToken(1));
		else
			return evalChildToken(0)->rightShift(*evalChildToken(1));
	}
};
...
public:
	...
	shared_ptr_to_constant eval()        //addition
	{
		/* ***** this needs work */
		if (this->stringValue[0] == '+')
			return evalChildToken(1)->addition(*evalChildToken(0));
		else
			return evalChildToken(0)->subtraction(*evalChildToken(1));
	}
};
...
public:
	...
	{
		//return(stringValue);
		throw std::runtime_error("string evaluation unimplemented");
	}
...
public:
    ...
    //should have 3 shared pointer to constant for different types
	...
	shared_ptr_to_constant postfix_increment()                //implement of ++
	{ // a ++;
		T temp = value;
		value++;
		return create(temp);
    ...
    shared_ptr_to_constant postfix_decrement()                    //implement of --
    { // a --;
        T temp = value;
        value--;
        return create(temp);
    ...
    shared_ptr_to_constant addition(constant& rval)                 //implement of +
    {
        return create(this->getValue() + rval.getIntValue());
    }

    shared_ptr_to_constant subtraction(constant& rval)              //implement of -
    {
        return create(this->getValue() - rval.getIntValue());
    }
...
public:
	...
	shared_ptr_to_constant complement()                     //implement of complement
	{
		/* ***** this  needs work */
		return create(-this->getValue() - 1);
	}
	...
	shared_ptr_to_constant leftShift(constant& right)              //shift operation.
	{
		/* ***** this  needs work */
		int power = 1;
		int times = right.getIntValue() % 32;
		for(; times > 0; times--)
			power *= 2;
		return create(this->getValue() * power);
	}

    shared_ptr_to_constant rightShift(constant& right)          //shift right
    {
        /* ***** this  needs work */
    	int power = 1;
    	int times = right.getIntValue() % 32;
