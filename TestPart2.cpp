	...
	for(unsigned int i=0; i<min(tokens.size(),tokenObjects.size()); i++)
	{
		...
		if(type==tokens.at(i).tokenType) {
			if(type=="ensc251::numeric_int_const<int>")
			{
				...
				BOOST_CHECK_EQUAL(intToken->getLineNumber(), tokens.at(i).lineNumber);
				BOOST_CHECK_EQUAL(intToken->getIndexNumber(), tokens.at(i).indexNumber);
			}
			else if(type=="ensc251::numeric_const<float>")
			{
				...
				BOOST_CHECK_EQUAL(floatToken->getLineNumber(), tokens.at(i).lineNumber);
				BOOST_CHECK_EQUAL(floatToken->getIndexNumber(), tokens.at(i).indexNumber);
			}
			else if(type=="ensc251::numeric_int_const<char>")
			{
				...
				BOOST_CHECK_EQUAL(charToken->getLineNumber(), tokens.at(i).lineNumber);
				BOOST_CHECK_EQUAL(charToken->getIndexNumber(), tokens.at(i).indexNumber);
			...
			else
			{
				...
				BOOST_CHECK_EQUAL(strToken->getLineNumber(), tokens.at(i).lineNumber);
				BOOST_CHECK_EQUAL(strToken->getIndexNumber(), tokens.at(i).indexNumber);
			}
		}
		else {
...
BOOST_AUTO_TEST_CASE( test1 ){
	...
	vector<tokenStruct> tokens = {{"int", 0, 0, "ensc251::type_spec"},
							   ...
							   {"2", 0, 8, "ensc251::numeric_int_const<int>"},
...
BOOST_AUTO_TEST_CASE( test2 ){
	...
	vector<tokenStruct> tokens = {{"char", 0, 0, "ensc251::type_spec"},
								  ...
								  {"'c'", 0, 9, "ensc251::numeric_int_const<char>"},
...
BOOST_AUTO_TEST_CASE( test3 ){
	...
	vector<tokenStruct> tokens = {{"float", 0, 0, "ensc251::type_spec"},
								  ...
								  {"5", 0, 35, "ensc251::numeric_int_const<int>"},
...
BOOST_AUTO_TEST_CASE( test4 ){
	...
	vector<tokenStruct> tokens = {{"char", 0, 0, "ensc251::type_spec"},
								  ...
								  {"10", 0, 19, "ensc251::numeric_int_const<int>"},
								  {":", 0, 22, "ensc251::punctuator"},
								  {"20", 0, 24, "ensc251::numeric_int_const<int>"},
...
BOOST_AUTO_TEST_CASE( test5 ){
	...
	vector<tokenStruct> tokens = {{"errno", 0, 0, "ensc251::int_id"},
								  ...
								  {"'2'", 0, 10, "ensc251::numeric_int_const<char>"},
								  {">>", 0, 14, "ensc251::shift_operator"},
								  {"5", 0, 17, "ensc251::numeric_int_const<int>"},
...
BOOST_AUTO_TEST_CASE( test6 ){
	...
	vector<tokenStruct> tokens = {{"float", 0, 0, "ensc251::type_spec"},
								  ...
								  {"=", 1, 3, "ensc251::gen_assignment_operator"},
								  {"39E-2", 1, 5, "ensc251::numeric_const<float>"},
								  {"-", 1, 11, "ensc251::additive_operator"} ,
								  {"(", 1, 13, "ensc251::punctuator"} ,
								  {"_f", 1, 15, "ensc251::numeric_id"},
								  {"++", 1, 18, "ensc251::postfix_operator"},
								  {")", 1, 21, "ensc251::punctuator"},
								  {";", 1, 23, "ensc251::punctuator"}};
	check(input, tokens);
}
